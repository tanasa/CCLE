...
# chooseCRANmirror(78)
# A script that computes the a graph metrics based on the set of fusions extracted from MCF-7 dataset that is part of CCLE collection. 
# The scripts computes the following : 
# Network measures (degree centrality, betweeness centrality, closeness centrality, etc), community detection, average path length, clustering coefficients etc
# The visualization is provided by the packages : VisNetworkData, network3D
# GO and pathway analysis of the network of fusions (with clusterprofiler)
# The fitting of the node degree to a continuous or discrete distribution is achieved with "gamlss" and  "fitdistrplus"
# the area under eCDF curve
################################################################################################
################################################################################################
################################################################################################
################################################################################################
# we work with the file C...mo.txt, where we have replaced "gene" with "Gene" ; hg19
...
# we may switch the chromosome sizes to hg19 (I guess that CCLE data is provided on hg19).  
...
el = y[!(y$Gene1=="") & !(y$Gene2==""), ]  #################################################### it is appropriate for the LIST of FUSIONS ; 
...
###############################################################################################
###############################################################################################
############################################################################################### we work with the dataframe that we call "el"
...
png(paste("graph",TUMOR_TYPE,"network.igraph.layout.kk.png", sep="."), width = 1600, height = 1600, units = "px")        
plot.igraph(g, 
	          # layout=lo*10,
...
png(paste("graph",TUMOR_TYPE,"network.igraph.layout.kk.CENTRALITY.DEGREE.png", sep="."), width = 1600, height = 1600, units = "px")        
plot.igraph(g, 
	         # layout=lo*10,
			 # layout = layout_in_circle(network),
	         edge.arrow.width = .25,
...
# betweenness centrality, which is defined as the number of geodesic paths (shortest paths) that go through a given node. 
...
png(paste("graph",TUMOR_TYPE,"network.igraph.layout.kk.COMMUNITY.spectral.clustering.png", sep="."), width = 2400, height = 2400, units = "px")        
plot(eb1, g, 
	         # layout=lo*10,
			   # layout = layout_in_circle(network),
	         edge.arrow.width = .25,
...
png(paste("graph",TUMOR_TYPE,"network.igraph.layout.kk.COMMUNITY.louvain.clustering.png", sep="."), width = 2400, height = 2400, units = "px")        
plot(eb2, g, 
	         # layout=lo*10,
			   # layout = layout_in_circle(network),
	         edge.arrow.width = .25,
...
# Assortment
# One major pattern common to many social networks (and other types of networks) is homophily or assortment—the tendency for nodes that 
# share a trait to be connected. The assortment coefficient is a commonly used measurement of homophily. 
...
#################################################################################################################### 
#################################################################################################################### 
#################################################################################################################### another way to compute the eCDF
...
# Since Riemann Sum is only an approximation, this deviates from 1 (total probability) a little bit. We
...
png(paste("graph",TUMOR_TYPE,"network.igraph.layout.kk.zoo.DENSITY.zoo.png", sep="."), width = 400, height = 400, units = "px")
plot(dens, lwd=3, type="l")
# abline(v=dens$x, lty="11")
dev.off()


png(paste("graph",TUMOR_TYPE,"network.igraph.layout.kk.zoo.eCDF.png", sep="."), width = 400, height = 400, units = "px")
